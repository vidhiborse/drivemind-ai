...
Camera Frame
&nbsp;&nbsp;&nbsp;→ Perception Layer (Face Detection + Recognition, Eyes, Mouth, Head Pose, Emotion, Objects, Seatbelt, Lanes, Vehicles/TTC)
&nbsp;&nbsp;&nbsp;→ Feature Aggregator (merges all module outputs into one state snapshot)
&nbsp;&nbsp;&nbsp;→ Decision Engine — rule-based v1, now upgraded to an **ML-based Risk Engine** (LightGBM, trained on logged decisions) with SHAP explainability; also flags unrecognized/unregistered drivers
...
| Face recognition / driver identity | DeepFace (embedding-based matching) |
...
Enrolled driver profile photos and their DeepFace-derived embedding cache files are **excluded from version control** (see `.gitignore`) and are never committed to this repository. Driver enrollment happens locally via `scripts/enroll_driver.py` and the resulting files stay on the local machine only.
...
Enroll yourself as a registered driver (one-time, required for face recognition):
